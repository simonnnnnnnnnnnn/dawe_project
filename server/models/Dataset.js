...
class Dataset extends Crud_Basics{
    get model(){
        return "dataset";
    }
    ...
    validate(record){
        if (!record.dataset_ID || typeof record.dataset_ID !== 'string'){
            throw new Error("dataset_ID must be a non-empty string");
        }
        for (const field of ['summary', 'organism', 'citation', 'platform', 'reference_series', 'value_type']){
    ...
    */
    ...
    async getProfiles(datasetID){
        return this.storage.search('profile', {dataset_ID: datasetID});
    }
}
